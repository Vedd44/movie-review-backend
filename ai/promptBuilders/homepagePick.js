...
const buildPickRankerPrompts = ({ preferences, intent, candidates }) => {
  ...
  return {
    ...
    userPrompt: [
      ...
      `Candidate pool:\n${candidateBlock}`,
      "Task:",
      "1. Choose exactly 1 top pick and 3 backup picks.",
...
const buildPickWriterPrompts = ({ preferences, intent, primary, backups }) => {
  ...
  return {
    ...
    userPrompt: [
      ...
      "6. Give each backup a short editorial role label and one-line rationale naming its real difference from the primary. Every backup must offer a different reason to choose it.",
